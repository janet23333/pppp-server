...
cmdb_agent="sh $file_path/cmdb-agent.sh"
service="sh $file_path/service.sh"
...
backup_logs() {
    ...
    date=$(date +%F-%H-%M)
    if [ -z "$project_name" ];then
        msg "[error] [func:backup_logs] 请指定project_name！"
		    ...
		    for project_name in *;do
				...
				if [[ -d ${dir} && ${dir} =~ "webapps" ]];then
                     if echo "${version}"|grep "^[0-9]\{6,\}$" > /dev/null;then
                          ...
                          rversion=`curl -s  $source_url/inventory/${version} | grep "${project_name}\ " | sort -u|head -n1|awk '{print $NF}'`
    ...
    if [ -n ${version} ] ;then
        ...
        for project_name in `echo ${project_name_list}|sed 's/;/ /g'`;do
            ...
            if echo ${version} | grep "^[0-9]\{6,\}$" >/dev/null ;then
                ...
                version=`curl -s  $source_url/inventory/${version} | grep "${project_name}\ " | sort -u|head -n1|awk '{print $NF}'`
            fi
            if [ ! -d "${project_sour_dir}/${version}/${project_name}" ];then
